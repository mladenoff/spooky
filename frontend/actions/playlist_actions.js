...
export const receivePlaylist = (id, payload) => ({
  type: RECEIVE_PLAYLIST,
  payload,
});

export const playPlaylist = (id, payload) => ({
  type: PLAY_PLAYLIST,
  id,
  payload,
});
...
export const requestPlaylist = id => (dispatch) => {
  ...
  return APIUtil.fetchPlaylist(id).then(
    playlist => (dispatch(receivePlaylist(id, playlist))),
  );
};
...
export const requestPlaylistPlayback = id => (dispatch) => {
  ...
  return APIUtil.fetchPlaylist(id).then(playlist => (
    dispatch(playPlaylist(id, playlist))),
  );
};
